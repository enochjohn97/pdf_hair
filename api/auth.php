...
switch ("$method:$action") {
    ...
    case 'POST:login':
        ...
        if (!$email || !$pass) jsonResp(['error' => 'Email and password are required'], 422);
    ...
    case 'GET:me':
        bootSession();
        $user = currentUser();
        if (!$user) jsonResp(['error' => 'Not authenticated'], 401);
    ...
    case 'POST:change-password':
        ...
        if (strlen($new) < 8) jsonResp(['error' => 'New password must be at least 8 characters'], 422);
